...
int saveFlightBytesDetails(const Flight* pFlight, FILE* fp)
{
	if (fwrite(pFlight->originCode, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
		return 0; //the meth that open fp need to close fp
	if (fwrite(pFlight->destCode, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
		return 0; 
	int pBuffer=0;
	saveToBufferFlightBytesDetails(pFlight, &pBuffer);
	if (fwrite(&pBuffer, sizeof(int), 1, fp) != 1)
		return 0;
	return 1;
}
int loadFlightBytesDetails(Flight* pFlight, FILE* fp)
{
	int buffer;
	if (fread(pFlight->originCode, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
		return 0; 
	pFlight->originCode[CODE_LENGTH] = '\0';
	if (fread(pFlight->destCode, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
		return 0;
	pFlight->destCode[CODE_LENGTH] = '\0';
	if (fread(&buffer, sizeof(int), 1, fp) != 1)
		return 0;
	loadFromBufferFlightBytesDetails(buffer, pFlight);
	return 1;
}

void saveToBufferFlightBytesDetails(const Flight* pFlight, int* pBuffer)
{
	*pBuffer= (pFlight->date.year << 14) | ((pFlight->date.month << 10) & 0x3C00) | ((pFlight->date.day << 5) & 0x3E0) | (pFlight->hour & 0x1F);
	// we are push bit to the end of pbuffer(in the end of int bits) and after use mask for each element  
}
void loadFromBufferFlightBytesDetails(int buffer, Flight* pFlight) //becuae block = 4 byte = int , it's easy convert with all the int and not byte byte
{
	pFlight->date.year = (buffer >> 14) & 0x3FFFF ;
	pFlight->date.month = buffer >> 10 & 0xF;
	pFlight->date.day = buffer >> 5 & 0x1F;
	pFlight->hour = buffer & 0x1F;
}